# GA1.1-team1O

Substract: code for substracting a number from an initial number.
Only works if the given numbers are integer or float.


the function add(a,b) in the file add.ipynb sums two numbers (a and b) and returns the result.

sum_list: a function for adding up all numbers in a list.
Only works if the given list has entries of type int and float.


Multiply code for multiplying two numbers.
the function uses two different inputs in order to give the choice to the user to pick two different numbers every time and returns the result
